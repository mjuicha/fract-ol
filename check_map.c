...
int check_walls(t_data *data)
{
    ...
    while (i < data->len_line)
    {
        ...
        while (data->map[i][j] && data->map[i][j] != '\n' && j < col)
        {
            if (i == 0 || i == data->len_line - 1 || j == 0 || j == col - 1)
            {
                if (data->map[i][j] != '1')
                    return (0);
            }
...
int check_valid_path(t_data *data)
{
    ...
    map = data->map;
    if (!get_player_pos(map, data->len_line, &p_row, &p_col))
        return (0);
    temp_map = malloc(sizeof(char *) * (data->len_line + 1));
    if (!temp_map)
        return (0);
    copy_map(map, temp_map, data->len_line);
    // print_map(temp_map);
    // printf("\n\n");
    ...
    while (i < data->len_line)
    {
        ...
        {
            free_maps(temp_map, data->len_line);
            return (0);
        }
        i++;
    }
