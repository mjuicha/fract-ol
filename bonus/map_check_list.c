...
/*   Updated: 2024/08/20 19:24:26 by mjuicha          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long.h"

void	flood_fill_c(char **map, int y, int x, t_data *data)
{
	...
	if (map[y][x] == '1' || map[y][x] == 'X' || map[y][x] == 'E')
		return ;
	map[y][x] = 'X';
	flood_fill_c(map, y, x - 1, data);
	flood_fill_c(map, y, x + 1, data);
	flood_fill_c(map, y - 1, x, data);
	flood_fill_c(map, y + 1, x, data);
}
...
int	check_coins_path(t_data *data)
{
	...
	flood_fill_c(temp_map, data->py, data->px, data);
	i = 0;
	while (i < data->row)
	{
